...
def show_fps(frame, fps):
    ...
    cv2.rectangle(frame, (x, y), (x+w, y+h), (30, 30, 30), -1)  # Dark background
    cv2.rectangle(frame, (x, y), (x+w, y+h), (80, 80, 80), 1)   # Thin border
    ...
    cv2.putText(frame, f"FPS: {fps:.1f}", (x+15, y+35), font, font_scale, (100, 255, 100), thickness)
...
if __name__ == '__main__':
    ...
    while cap.isOpened():  # Main loop to process video frames
        ...
        if success:  # If the frame is read successfully
            ...
            cv2.namedWindow("Person Counter", cv2.WND_PROP_FULLSCREEN)  # Create a named window
            cv2.setWindowProperty("Person Counter", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)  # Set the window to fullscreen
            cv2.imshow("Person Counter", resized_frame)  # Show the annotated frame    
...
# python person-counter.py --model yolo11n.pt --video hotel.mp4 --conf 0.5